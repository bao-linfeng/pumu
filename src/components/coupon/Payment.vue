<template>
  <div class="payment">
    <NavLink />
    <div class="center">
        <HeadTitle :title="'购买谱劵'" class="about_title" />
        <div class="payment_form">
            <label for="">购买金额</label>
...
export default {
    ...
    methods:{
        ...
        weixinPayment:async function(){//微信支付请求
            let data=await api.postAxios('pay/purchase',{goodsType:'购买谱券', fee:Number(this.payment),userKey:this.userKey,"trade_type":this.isIphone ? "MWEB" : "NATIVE",'orderId':this.orderId});
        ...
        AliPayment:async function(){//支付宝支付请求
            let data=await api.postAxios('pay/purchaseAli',{'cao':window.location.origin+'/couponPaymentOk',goodsType:'购买谱券', fee:Number(this.payment),userKey:this.userKey,"trade_type":this.isIphone ? "wap" : "page",'orderId':this.orderId});
            if(data.statusCode == 200){
