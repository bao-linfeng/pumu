...
export default {
    ...
    methods:{
        ...
        getAliPayment:async function(){
            let data=await api.getAxios('https://gdata.qingtime.cn/rechargeDetail?token='+this.token+'&donateKey='+this.out_trade_no+'&simple=1',true);
            if(data.statusCode == 200){
                if(data.result.status == 2){
                    this.$router.push('/mydonation')
                }else{
                    this.$router.push('/')
                }
