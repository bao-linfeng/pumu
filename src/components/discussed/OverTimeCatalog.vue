    ...
    <div class="over-time-wrap">
        <div class="over-time-box">
            ...
            <div class="content-box">
                <p class="red">(提示:你正在进行批量设置逾期谱.)</p>
                <p class="content">当前共计选中 {{list.length}}条数据，符合更改条件，可以批量设置逾期。</p>
                <div class="input-box">
...
export default {
    ...
    methods:{
        ...
        async patchOverTimeBatch(){// 批量处理逾期谱
            let data = await api.patchAxios('v3/review/catalog/overTimeBatch',{'catalogKeyArray': this.list, 'siteKey': this.stationKey, 'userKey': this.userId});
            if(data.status == 200){
                this.$XModal.message({ message: '批处理逾期谱成功', status: 'success' })
                this.close();
            }else{
