        ...
        <div class="content-box">
            <ul class="edit-content">
                <li :class="{w100: item.w, marginL10: item.marginL10}" v-for="(item, index) in argumentsList" :key="index" v-show="!item.hide">
                    ...
                    <!-- red: item.required && !parameter[item.fieldName],  -->
                    <el-input class="w90" :class="{changeActive: changeFieldArr.indexOf(item.fieldName) > -1, w80: ['place'].indexOf(item.fieldName) > -1, w33: item.fieldName == 'surname'}" v-model="parameter[item.fieldName]" :title="parameter[item.fieldName]" :clearable="item.clearable"  :disabled="item.disabled || read" size="medium"></el-input>
    ...
    data: () => {
        return {
            argumentsList: [
                ...
                {'fieldMeans': '谱名', 'fieldName': 'genealogyName', 'required': true,'clearable': true, 'w': '100%', 'rule': '可以放半型“.“”( )”，不可以有其他標點符號，不可有空格，可拆字（），不可有阿拉伯數字'},
                {'fieldMeans': '出版年', 'fieldName': 'publish', 'required': true,'clearable': true, 'rule': '只能填入阿拉伯數字，不可加“年”'},
                ...
                {'fieldMeans': '谱籍地(现代)', 'fieldName': 'place', 'clearable': true, 'required': true, 'w': '100%', 'rule': '只能錄入一個完整地名，含省份，市，縣，鄉/鎮/街道，如果有村可錄入；'},
                {'fieldMeans': '谱籍地(原谱)', 'fieldName': 'LocalityModern', 'clearable': true, 'required': true, 'w': '100%', 'rule': '不同的行政區劃需用","分開，只能放文字，只能放大寫數字，不可有空格,不可有其他標點符號,除了拆字（）'},
                {'fieldMeans': '总卷数', 'fieldName': 'volume', 'required': true, 'clearable': true, 'rule': '依照譜書目錄填入卷（冊）數，可以使用","，不可有空格，不分卷全____冊'},
                {'fieldMeans': '缺卷说明', 'fieldName': 'lostVolume', 'clearable': true, 'marginL10': 10, 'required': true, 'rule': '提供錄入選單如下: 選項1：總卷數欄位如為總卷數不詳，填"存卷______________”；選項2：總卷數欄位如為不分卷且確定不全，填"存___冊”；選項3：不缺卷（冊）；選項4：知道總卷數且確定不全，則填“缺。。。”；選項5：其他（可自由填入資訊）只可以使用半型“,“~“，數字必須是阿拉伯數字，不可有空格；'},
                {'fieldMeans': '可拍册数', 'fieldName': 'hasVolume', 'clearable': true, 'required': true, 'rule': '只要填阿拉伯數字，沒有文字'},
                {'fieldMeans': '实拍册数', 'fieldName': 'volumeNumber', 'marginL10': 10, 'disabled': true, 'rule': '只要填阿拉伯數字，沒有文字'},
                {'fieldMeans': '作者', 'fieldName': 'authors', 'clearable': true, 'required': true, 'rule': '只能放1個人的完整姓名（姓+名），最後一個字元不可以是公，除了拆字（），不可有其他標點符號；可寫不詳'},
