<template>
  <div class="wrap">
    ...
    <div class="content">
      ...
      <div class="search_wrap" v-show="isUnique>=2" :class="{marginBottom20:isUnique==2}">
        <h3 class="title"><i @click="goBackMyGenealogy">家谱检索</i> / <span>查重</span></h3>
        <div class="search_box2">
          ...
          <div class="search_box2_bottom">
            <label class="label_2">查重项</label>
            <el-checkbox v-model="unique_surname">姓氏</el-checkbox>
            <el-checkbox v-model="unique_name">谱名</el-checkbox>
            <el-checkbox v-model="unique_place">谱籍地</el-checkbox>
            <el-checkbox v-model="unique_hall">堂号</el-checkbox>
            <el-checkbox v-model="unique_shizu">始祖</el-checkbox>
            <el-checkbox v-model="unique_version">版本年号</el-checkbox>
            <el-checkbox v-model="unique_author">修撰者</el-checkbox>
            <el-checkbox v-model="unique_time">修撰时间</el-checkbox>
            <el-checkbox v-model="unique_volume">卷数</el-checkbox>
            <el-checkbox v-model="unique_page">页数</el-checkbox>
            <div class="btn width100" @click="getCheckDuplication">
              <span>开始查重</span>
            </div>
          </div>
        </div>
      </div>
      <div class="btn_wrap" v-show="isUnique==1">
        <div class="btn_wrap_left">
          <div class="btn_wrap_toolbar">
            <span :class="'icon'+index+(index == 2 ? ' hide' : '')" v-for="(item,index) in ['添加','导入','查重','删除']" :key="'toolbar'+index" @click="clickToolbar(index)">{{item}}</span>
          </div>
          <input class="btn cnki" type="button" value="查重" @click="clickToolbar(2)" />
          <!-- <input class="btn root" type="button" value="分级公开" @click="openRootModal" />
          <input v-if="siteKey == '593133135'" class="btn swap" type="button" value="家谱分发" @click="handleClickToolbar(1)" />
          <input v-else class="btn swap" type="button" value="交换家谱" @click="openSwapModal" /> -->
          <!-- <input class="btn gray checkRepeat" type="button" value="与'时光谱库'查重" /> -->
        </div>
        <!-- <input class="btn green" type="button" value="站点透视" @click="openSiteModal" /> -->
      </div>
      <div class="table_wrap" v-show="isUnique<=2">
        <div class="title">
          <h3>检索结果</h3>
          <div>
            <el-checkbox :value="allSitePerspective" @change="allSitePerspectiveCheck(allSitePerspective)">选择全部检索结果</el-checkbox>
      ...
      <div class="table_wrap margintop20" v-show="isUnique==3">
        <div class="title">
          <h3>共有{{checkDuplication.length}}本目标家谱有重复，共{{checkDuplicationCount}}条记录与您选择的谱目比较接近，请仔细核实后再继续</h3>
          <el-button type="primary" size="mini" class="marginright20" @click="exportReport">导出报告</el-button>
        </div>
        <div class="table_box style1">
          <div class="checkDuplication_wrap">
            <div class="checkDuplication_thead">
              <span v-for="(item,index) in ['序号','家谱编码','条目来源','谱名','姓氏','谱籍地','堂号','始祖','版本年号','修撰时间','修撰者','卷数','页数','家谱影像','创建人','上传时间','重复处','操作']" :key="'checkDuplication_thead'+index">{{item}}</span>
            </div>
            <ul class="checkDuplication_body_ul">
              <li class="checkDuplication_body_li" :class="{'redbColor':item.isRepeat === true,'greenbColor':item.isRepeat === false}" v-for="(item,index) in checkDuplication" :key="'checkDuplication'+index">
                <span>{{index}}</span>
                <span>{{item._key}}</span>
                <span>{{item.source}}</span>
                <span>{{item.genealogyName}}</span>
                ...
                <span>{{item.hasImage ? '有影像' : '无影像'}}</span>
                <span>{{item.creator}}</span>
                <span>{{item.createTime | getLocalTime}}</span>
                <span>--</span>
                <p>
                  <span :class="{'disabled':!item.hasImage}" @click="readView(item)">阅读</span>
                  <i>|</i>
                  <span class="redColor" :class="{'disabled':item.isRepeat === true}"  @click="setRepeat(item,true)">标记重复</span>
                  <i>|</i>
                  <span class="greenColor" :class="{'disabled':item.isRepeat === false}" @click="setRepeat(item,false)">标记不重复</span>
                  <i>|</i>
                  <span @click="deleteAction(item._key,item.siteKey == siteKey)">删除</span>
                </p>
                <ul class="checkDuplication_body_ul">
                  <li class="checkDuplication_body_li checkDuplication_body_li2" v-for="(item2,index2) in item.similar" :key="'checkDuplicationSimilar'+index2">
                    <span></span>
                    <span>{{item2._key}}</span>
                    <span>{{item2.source}}</span>
                    <span>{{item2.genealogyName}}</span>
                    <span>{{item2.surname}}</span>
                    <span>{{item2.place}}</span>
                    <span>{{item2.hall}}</span>
                    <span>{{item2.ancestor}}</span>
                    <span >{{item2.version}}</span>
                    <span>{{item2.writeTime}}</span>
                    <span>{{item2.author}}</span>
                    <span>{{item2.volume}}</span>
                    <span>{{item2.pages}}</span>
                    <span>{{item2.hasImage ? '有影像' : '无影像'}}</span>
                    <span>{{item2.creator}}</span>
                    <span>{{item2.createTime | getLocalTime}}</span>
                    <span>{{Number(item2.surname == item.surname)+Number(item2.genealogyName==item.genealogyName)+Number(item2.place==item.place)+Number(item2.hall==item.hall)+Number(item2.ancestor == item.ancestor)+Number(item2.version==item.version)+Number(item2.author==item.author)+Number(item2.writeTime==item.writeTime)+Number(item2.volume==item.volume)+Number(item2.pages==item.pages)}}</span>
                    <p>
    ...
    <!-- <ul class="toolbar hide">
      <li :title="'交换记录'" @click="handleClickToolbar(0)"></li>
      <li v-if="siteKey == '593133135'" :title="'家谱分发'" @click="handleClickToolbar(1)"></li>
      <li v-if="siteKey == '593133135'" :title="'分发历史'" @click="handleClickToolbar(2)"></li>
    </ul> -->
    <SwapListModal v-if="isSwapListModal" v-on:close-site="isSwapListModal=false" v-on:save-site="getSwap" />
    ...
    <LeadModal v-if="isLeadShow" v-on:close-lead="isLeadShow=false" v-on:save-lead="saveLead" />
    <GenealogyModal v-if="isGenealogyShow" :type="isEdit" :data="[list[isEdit]]" v-on:close-genealogy="isGenealogyShow=false;isEdit=-1" v-on:save-genealogy="saveGenealogy" />
    ...
    <PerspectiveListModal v-if="isPerspectiveList" v-on:close-perspective="isPerspectiveList=false" />
    <SourceModal v-if="isSourceModal" v-on:close-source="isSourceModal=false" :gid="gid" :gvolume="gvolume" />
    <SourcePrevModal v-if="isSourcePrevModal" v-on:close-source="isSourcePrevModal=false" :gid="gid" />
...
import LeadModal from "../../components/myGenealogy/LeadModal.vue";
import GenealogyModal from "../../components/myGenealogy/GenealogyModal.vue";
import SitePerspectiveModal from "../../components/myGenealogy/SitePerspectiveModal.vue";
import PerspectiveListModal from "../../components/myGenealogy/PerspectiveListModal.vue";
import SourceModal from "../../components/myGenealogy/SourceModal.vue";
...
import SwapListModal from "../../components/myGenealogy/SwapListModal.vue";
...
.wrap{
  ...
  .content{
    ...
    .cnki{
      background:#fff url('../../assets/icon/unique.svg') 35px 50% no-repeat;
      background-size: 16px;
      text-indent: 20px;
      margin-right: 0;
    }
